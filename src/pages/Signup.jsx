...
export default function Signup() {
  ...
  return (
    <div className="signup px-5 pt-10 pb-5 rounded flex flex-col gap-5 items-center bg-gray-950 w-[min(98%,550px)]">
      <h1 className="text-center font-extrabold text-3xl mb-5 text-white">
         أدمن جديد
      </h1>
      <form  className="flex flex-col gap-5" onSubmit={handelSignup}>
        ...
        >
          ...
          <div className="text-sm text-white self-start">
        ...
        <button
          type="submit"
          disabled={loading}
          className={`bg-gray-200 text-gray-950  hover:bg-sky-500 transition-all duration-300 ease-out px-5 py-2 rounded cursor-pointer font-semibold ${
            loading && "opacity-50"
          }`}
        >
          تسجيل
      ...
      <div className="flex flex-col gap-3 items-center">
        <p className="text-white">هل لديك حساب مسبقاً ؟</p>
        <Link to="/login">
          <button className="bg-white text-black hover:text-white hover:bg-sky-800 transition-all duration-300 ease-out px-5 py-2 rounded cursor-pointer">
            تسجيل دخول
          </button>
        </Link>
      </div>
