...
export default function Login() {
      ...
      if (isEmailVailditon) {
        ...
        try {
          ...
          if (error.code === "auth/user-not-found") {
            ...
            setShowPopup(true);
            setErrorMsg("هذا الحساب غير موجود");
          }
        } finally {
          setIsLoading(false);
        }
  ...
  return (
    // wrapper
    <div className="login px-5 py-10 rounded flex flex-col gap-5 items-center bg-gray-950 w-[min(98%,450px)]">
      {/* title */}
      ...
      <form onSubmit={handelLogin}  className="flex flex-col gap-5 w-full ">
        ...
        <div className="flex justify-center gap-8">
          <button
            type="submit"
            disabled={loading}
            className={`bg-gray-200 text-gray-800 hover:bg-sky-500 transition-all duration-300 ease-out px-5 py-2 rounded cursor-pointer font-semibold ${
              loading && "opacity-50"
            }`}
          >
            تسجيل دخول
          ...
          <Link to="/signup">
            <button className="bg-white text-gray-800 hover:text-white hover:bg-sky-700 transition-all duration-300 ease-out px-5 py-2 rounded cursor-pointer font-semibold">
              تسجيل جديد
            </button>
          </Link>
        </div>
